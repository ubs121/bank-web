...
st += "<IMG SRC=/prospect/images/"+i+".gif BORDER=1 HSPACE=3>"+this.legends[i]+"<BR>\n";
}
st += "</FONT></TD></TR></TABLE>";
return st;
}
...
str += "<IMG BORDER=0 SRC=\"/prospect/images/"+i+".gif\" ";
//alert(str)
str += "ALT=\"";
...
str += "<IMG SRC=\"/prospect/images/clear.gif\" WIDTH="+parseInt(g.hscale)+" HEIGHT=5>";
str += "</TD>\n<TD VALIGN=BOTTOM";
if(g.bgColor) str += " BGCOLOR=\""+g.bgColor+"\"";
str += ">";
}
str += "<IMG SRC=\"/prospect/images/clear.gif\" WIDTH=1 HEIGHT=5>";
}
str += "</TD>\n";
}
...
str += "<IMG SRC=\"/prospect/images/black.gif\" HEIGHT=1 WIDTH=";
...
for(var i = 0; i < g.rows.length; i++) {
if(parseInt(g.vscale*g.rows[i][j]) < 0) {
str += "<IMG VSPACE=0 HSPACE=0 BORDER=0 ALIGN=TOP SRC=/prospect/images/"+i+".gif WIDTH="+
...
str += "<IMG SRC=\"/prospect/images/clear.gif\" ALIGN=TOP BORDER=0 WIDTH="+parseInt(g.hscale)+
" HEIGHT=5>";
str += "</TD>\n<TD VALIGN=TOP";
if(g.bgColor) str += " BGCOLOR=\""+g.bgColor+"\"";
str += ">";
}
str += "<IMG SRC=\"/prospect/images/clear.gif\" ALIGN=TOP WIDTH=1 BORDER=0 HEIGHT=5>";
}
str += "</TD>\n";
if(g.legends && g.posMax == 0) {
...
//rgm mod 10 May 00
//Years in 2000 are displayed with no leading 0 so 2001 is "21 jan 1" or 2000 "21 jan 0" etc
maxval = dateStr.length;
teststr = dateStr.charAt(maxval-2);

//If date is Jan-0
//if(teststr == "-") {
//	if(dateStr.charAt(maxval-1) == "0") {
//		dateStr += "0" ;
//	} else {
//		//If date is Jan-1 or Jan-2 etc (for 2001 2002 etc)
//		newdateStr = "";
//		for (j = 0; j < maxval - 2; j++) {
//			newdateStr += dateStr.charAt(j) ;
//		}
//		newdateStr += "-0" ;
//		newdateStr += dateStr.charAt(maxval-1) ;
//		dateStr = newdateStr;
//	}
//}
//rgm end of mod
...
st += "<IMG SRC=/prospect/images/black.gif HEIGHT=10 WIDTH=1>";
st += "<IMG SRC=/prospect/images/clear.gif HEIGHT=1 WIDTH=1>";
var n = g.rows[0].length;
var mult = g.rows.length;
if(g.stacked || g.relative) mult = 1;
for(var i = 0; i < n; i++) {
st += "<IMG SRC=/prospect/images/clear.gif HEIGHT=1 WIDTH="+(g.hscale*mult)+">";
if((i+1) % g.skip)
st += "<IMG SRC=/prospect/images/clear.gif HEIGHT=10 WIDTH=1>";
else
st += "<IMG SRC=/prospect/images/black.gif HEIGHT=10 WIDTH=1>";
}
st += "</TD></TR><TR>\n";
...
//if(h < 15) {
//if(!g.posMax)
//alert("Warning! The scale for this graph is too small to display.");st += "<TD></TD><TD></TD><TD></TD>\n"
//return st;
//}
...
st += "<IMG SRC=/prospect/images/clear.gif WIDTH=1 HEIGHT="+H+"><BR>\n";
}
st += "</TD>\n";
st += "<TD VALIGN=TOP>";
for(var i = 0; i < n; i++) {
st += "<IMG SRC=/prospect/images/clear.gif WIDTH=1 HEIGHT="+(h-1)+"><BR>\n";
st += "<IMG SRC=/prospect/images/black.gif WIDTH=6 HEIGHT=1><BR>\n";
}
st += "</TD>\n";
st += "<TD VALIGN=TOP>";
st += "<IMG SRC=/prospect/images/black.gif WIDTH=1 HEIGHT="+(g.height*p)+">";
st += "<IMG SRC=/prospect/images/clear.gif WIDTH=1 HEIGHT="+(g.height*p)+">";
st += "</TD>\n"
return st;
}
...
str += "<IMG SRC=/prospect/images/0.gif WIDTH="+parseInt(g.hscale)+" HEIGHT="+(g.height-k)+" ";
str += "ALT=\"";
...
if(parseInt(g.vscale*g.rows[i][j]) > 0) {
str += "<IMG SRC=/prospect/images/"+i+".gif WIDTH="+parseInt(g.hscale)+" HEIGHT="+
...
if(!drawn) str += "<IMG SRC=/prospect/images/clear.gif WIDTH="+parseInt(g.hscale)+" HEIGHT=1>";
str += "</TD>\n";
str += "<TD";
if(g.bgColor) str += " BGCOLOR=\""+g.bgColor+"\"";
str += "><IMG SRC=/prospect/images/clear.gif WIDTH=1 HEIGHT=5></TD>\n";
}
}
if(g.legends && g.posMax != 0) {
str += "<TD WIDTH=5 NOWRAP ROWSPAN=3></TD>\n<TD ROWSPAN=3>";
