...
Sub Query_Click()
   ...
   Dim new_item,new_page,fname,tname,prefix,printdoc,mainfile
   ...
   mergefile=document.all.merge.value
   mainfile=document.all.mainfile.value
   subfile=document.all.subfile.value
   defaulting=dataform.defaulting.value
   docname=document.all.printdocnames.value
   no_update_delete=dataForm.no_update_delete.value
   on error goto 0
   new_page = "/prospect/asp/binput.asp?fn=" & fname &  "&fn1=" & trans_fname & "&tn=" & tname & "&mainfile=" & mainfile & "&subfile=" & subfile & "&item=" & prefix & new_item  & "&baditem=empty&blank=no&err=noerrors&found=yes&search=" & search & "&no_update_delete=" & no_update_delete & "&printdoc=" & printdoc & "&merge=" & mergefile  & "&docname=" & docname  & "&defaulting=" & defaulting
...
Sub pop_up
  ...
  prev_good=1
  testing=dataform.no_update_delete.value
...
'old server validation - errmsg = picklin.conv_validate(cstr(how), cstr(conversion), cstr(datum), cstr(tfile), cstr(v7), cstr(v36), cstr(max_length))
          select case conversion
			 ...
			 if IsNumeric(datum) then
			 	if len(datum) = 6 then
					dday = left(datum,2)
					mmonth = mid(datum,3,2)
					yyear = mid(datum,5,2)
					datum = dday & "/" & mmonth & "/" & yyear
				else
					msgbox "invalid date " & datum
					save_form_Data="1"
                    exit function	
				end if	
			 end if	
			 ...
			 if fdate<>"" then
                   ...
                   if conversion = "D2X" then 
                      internal_datum=Formatdatetime(datum,2)
                   else
                      internal_datum = DateDiff("d", "31/12/67", datum)
                   end if
               else
...
if Str="" then FormatNum="" : exit Function
If isnumeric(str) then
   if isnumeric(decimal) then formatnum=formatnumber(str,decimal) else formatnum=str
else
