...
<%
parameters=Request.cookies("TA1682")("parms")
...
<TR>
<TD CLASS="mm"><B>Date to which Discount Offered<BR>(leave null if no discount offered)</B>
<TD><input type="Text" name="6,0,0,Date to which Discount Offered,,D2" value="<%=parms(5)%>" align="LEFT" size="10" maxlength="10">

<TR>
<TD CLASS="mm"><B>Date at which penalty will be applied<BR>(leave null if no penalty applicable)</B>
<TD><input type="Text" name="7,0,0,Date at which penalty will be applied,,D2" value="<%=parms(6)%>" align="LEFT" size="10" maxlength="10">
...
'response.end
   Set tempobj = Server.CreateObject("cka_ta16.ta16")
   result = tempobj.TA1682(session("logentry"),0,0,"",cstr(sentence),cstr(after))
   set tempobj = nothing
