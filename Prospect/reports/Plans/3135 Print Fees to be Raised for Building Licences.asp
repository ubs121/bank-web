...
<%
parameters=Request.cookies("TA3135")("parms")
...
<TR>
<TD colspan=2>Fees will be raised for these selected PLANS where conditions are complete and fees not raised
...
if validated = "1" then
   ...
   itemlist=cka_ta31.TA3135(session("logentry"))
   Set cka_ta31 = nothing
   if left(itemlist,1) = chr(8) then
      response.write("<tr><td alogn=center>" & "Error " & mid(itemlist,2))
   else
      response.write("<TABLE WIDTH=60% border=1 cellspacing=3 cellpadding=1  align=center>")
