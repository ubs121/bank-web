...
if validated = "1" then
   ...
   dms=replace(dms,"fn=invoice","fn=cr.batch")
   title=replace(title,"INVOICE","CREDIT")
   itemlist = Picklin.making_html(Server.MapPath("\prospect\template\"),session("logentry"),"CR.BATCH", "SSELECT CR.BATCH WITH BATCH = """ & batchnum & """","credit", "1", "200", "0", "","",0,"",cstr(title),cstr(hds),cstr(tds),cstr(dms)) ' the 20 is the page size wanted
...
<%
parameters=Request.cookies("TA2421")("parms")
parms=split(parameters,chr(8))
redim preserve parms(10)
Dim selsent, items
selsent = "qselect dict cr.batch @batches (2"
