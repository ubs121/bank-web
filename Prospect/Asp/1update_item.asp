    ...
    if result="" then
       ...
       select case template
          ...
          case "iinvest_borrower"
             ...
             Set cka_loans=Nothing
          case "igen_l_grp_class"
             Set cka_loans=Server.CreateObject("cka_gl.gl")
             result = cka_loans.gl_grp_class_update(Cstr(session("logentry")),cstr(file), cstr(item),cstr(after),cstr(before),cstr(doing))
             Set cka_loans=Nothing
          case "igen_l"
             Set cka_loans=Server.CreateObject("cka_gl.gl")
             result = cka_loans.gl_update(Cstr(session("logentry")),cstr(file), cstr(item),cstr(after),cstr(before),cstr(doing))
             Set cka_loans=Nothing
          case "ijournal"
             Set cka_loans=Server.CreateObject("cka_gl.gl")
'Response.write(session("logentry") & "," & "file = " & file & "<BR>item = " & item & "<BR> after = " & after & "<BR>before = " & before & "<BR>doing = " & doing & "<BR>trnasrecbefore = " & transrec_before & "<BR>trnasrecafter = " & transrec_after)
'response.end
Response.write("Result:" & after)
Response.end 
		 ...
		 Set cka_loans=Nothing
          case "imaint"
             ...
             result = cka_loans.debtors_maint_invoice(Cstr(session("logentry")),cstr(file), cstr(item),cstr(after),cstr(before),cstr(transrec_before),cstr(transrec_after),cstr(doing))
		 Set cka_loans=Nothing
          case "istock_issue"
...
response.end
             ...
             result = cka_loans.issue(Cstr(session("logentry")),cstr(file), cstr(item),cstr(after),cstr(before),cstr(doing))
		 Set cka_loans=Nothing
          case "igen_l_grp"
             Set cka_loans=Server.CreateObject("cka_gl.gl")
             result = cka_loans.gl_grp_update(Cstr(session("logentry")),cstr(file), cstr(item),cstr(after),cstr(before),cstr(doing))
             Set cka_loans=Nothing
          case "igen_l_levmnt"
             Set cka_loans=Server.CreateObject("cka_gl.gl")
		...
		case "iinvoice"
     	  	 Set cka_loans=Server.CreateObject("cka_ta31.ta31")
             result = cka_loans.debtors_invoice(Cstr(session("logentry")),cstr(file),cstr(item),cstr(before),cstr(after),cstr(transrec),cstr(doing))
             Set cka_loans=Nothing			 
		  case else
...
'response.write("befor" & transfile)
'response.write("after" & after)
response.end

  page = "/prospect/asp/binput.asp?fn=" & file & "&fn1=" & transfile & "&tn=" & template & "&item=" & item & "&baditem=empty&blank=no" & "&err=" & dll_message & "&found=yes&search=" & search
